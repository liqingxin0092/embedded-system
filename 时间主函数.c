...
void HAL_RTCEx_RTCEventCallback(RTC_HandleTypeDef *hrtc)
{
    HAL_RTC_GetTime(hrtc, &time,RTC_FORMAT_BIN );
	  ...
	  OLED_ShowChinese(0, 0, "现在时间是");
	  OLED_Printf(0, 16, OLED_8X16,"%d/%02d/%02d ", 2000 + datebuff.Year, datebuff.Month, datebuff.Date);
	  OLED_Printf(0, 32, OLED_8X16,"%02d:%02d:%02d ", time.Hours, time.Minutes, time.Seconds);
	  OLED_Update();
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  __HAL_RCC_BKP_CLK_ENABLE();//使能电源时钟PW     //rtc相关
...
__HAL_RTC_SECOND_ENABLE_IT(&hrtc,RTC_IT_SEC);//开启秒更新中断
  ...
  if(HAL_RTCEx_BKUPRead(&hrtc,RTC_BKP_DR6)!= 0x5052)  //RTC时钟
    {
  	  HAL_RTCEx_BKUPWrite(&hrtc,RTC_BKP_DR6,0x5052);
  	  time.Hours=23;
  	  time.Minutes=59;
...
{
	HAL_RTCEx_BKUPWrite(&hrtc,RTC_BKP_DR1,0x5052);
